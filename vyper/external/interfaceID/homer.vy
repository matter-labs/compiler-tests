...
def supportsInterface(interfaceID: bytes4) -> bool:
    return interfaceID == method_id("supportsInterface(bytes4)") or interfaceID == convert(slice(convert(bitwise_xor(convert(method_id("is2D()"), uint256), convert(method_id("skinColor()"), uint256)), bytes32), 0, 4), bytes4) # ERC165 Simpson
